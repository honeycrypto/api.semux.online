...
const proxy = httpProxy.createProxy({
  target: CONFIG.SEMUX_API_ADDR,
  auth: `${CONFIG.SEMUX_API_USER}:${CONFIG.SEMUX_API_PASS}`,
  proxyTimeout: 15000,
});
...
const server = app.listen(
  CONFIG.API_SEMUX_ONLINE_PORT,
  CONFIG.API_SEMUX_ONLINE_HOST,
  () => {
    const address = server.address();
    console.log(
      `Server listening: http://${address.address}:${address.port}`,
      address
    );
  }
);
...
function modifySwaggerJsonMiddleware(version) {
  return (req, res, next) => {
    const auth = { user: CONFIG.SEMUX_API_USER, pass: CONFIG.SEMUX_API_PASS };
    const callback = (err, _, orig) => {
      if (err) next(err);
      else res.json(modifySwaggerJson(version, orig));
    };
    request.get(
      url.resolve(CONFIG.SEMUX_API_ADDR, req.originalUrl),
      { auth, json: true },
      callback
    );
  };
...
function modifySwaggerJson(version, orig) {
  ...
  const extraDescription =
    "**Semux Online API** project: https://github.com/witoldsz/api.semux.online";
  return R.pipe(
    R.assocPath(["info", "title"], "Semux Online API"),
    R.over(
      R.lensPath(["info", "description"]),
      (d) => `${d}\n\n${extraDescription}`
    ),
    R.assoc("paths", paths),
    R.dissoc("security"),
